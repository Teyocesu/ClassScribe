...
- [x] Fase 2D **FIXED / PARTIAL**: separar master source-rate/stereo durable (`master.raw` + manifest) del derivado ASR 16 kHz mono (`source.wav`); resampling master stateful con invariant acumulativo de frames, drain/flush determinista por generation/format con gap sin interpolación, propagación terminal de fallo durable al control plane y política explícita observable para process-loopback Windows; revisión independiente, runtime/compile Windows (**SKIPPED — dotnet/csc unavailable**), gates físicos Windows, CATap global físico macOS, TCC y hardware/device-change permanecen pendientes.
